...
const CameraCapture = ({ onCapture, onCancel, useBackCamera = false }) => {
    ...
    }, [setAvailableCameras, setCurrentCameraInfo]);
    ...
    }, [findPreferredDeviceId]); // Removed stream dependency
    ...
    }, [videoRef, canvasRef, setPhoto, setError, stopCamera]);
    ...
    useEffect(() => {
        ...
        const initializeCamera = async () => {
            ...
            try {
                ...
                if (mounted) {
                    let errorMessage = 'Camera initialization failed. Please try again.';

                    if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                        errorMessage = 'Camera permission denied. Please click the camera icon in your browser address bar and select "Allow".';
                    } else if (err.name === 'NotFoundError' || err.name === 'DevicesNotFoundError') {
                        errorMessage = 'No camera found on this device. Please ensure your camera is connected and not being used by another application.';
                    } else if (err.name === 'NotReadableError' || err.name === 'TrackStartError') {
                        ...
                        errorMessage = 'Camera access is not supported in this browser or in HTTP. Please use HTTPS or try a different browser like Chrome, Firefox, or Edge.';
                    }
                ...
                if (mounted) {
                    setIsInitializing(false); // Set initialization to false after attempt
                }
            }
        };
        ...
        return () => {
            mounted = false;
            // Stop any existing stream properly using ref
            if (videoRef.current && videoRef.current.srcObject) {
                const tracks = videoRef.current.srcObject.getTracks();
                tracks.forEach(track => track.stop());
            }
    ...
    return (
        <div className="camera-container">
            {error ? (
                ...
                <div className="camera-initializing" style={{
                    display: 'flex',
                    ...
                    <h3 style={{ margin: '0 0 var(--space-2) 0', fontSize: '1.25rem' }}>Accessing Camera</h3>
                    <p style={{ margin: 0, color: 'var(--text-secondary)' }}>Requesting permission to use your camera...</p>
